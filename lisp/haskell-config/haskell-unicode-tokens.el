;;; -*- coding: utf-8; -*-
;; haskell-unicode-tokens.el --- (No) Tokens for Unicode Tokens package
;;
...
;; unicode-tokens-<foo> is set from haskell-<foo>.  See the corresponding
;; variable for documentation.
;;
;; For Haskell, there is no dedicated token syntax, it's probably
...
(defconst haskell-token-format "%s")	; plain tokens
(defconst haskell-token-match nil)
(defconst haskell-hexcode-match nil)

(defun haskell-unicode-tokens-set (sym val)
  "Change a Unicode Tokens configuration variable and restart."
  (set-default sym val)
  (when (featurep 'haskell-unicode-tokens) ; not during loading
    (proof-unicode-tokens-configure)))

(defcustom haskell-token-symbol-map
  '(;; Greek letters
    ("alpha" "α")
    ("beta" "β")
    ("gamma" "γ")
  ...
  "Table mapping Haskell tokens to Unicode strings.
...
results will be undefined when files are saved."
  ...
  :set 'haskell-unicode-tokens-set
  :group 'haskell
  :tag "Haskell Unicode Token Mapping")

(defcustom haskell-shortcut-alist
...
meaning to be useful."
  ...
  :set 'haskell-unicode-tokens-set
  :group 'haskell
  :tag "Haskell Unicode Input Shortcuts")
...
(defconst haskell-control-char-format-regexp
  ;; FIXME: fix Haskell identifier syntax below
  "\\(\s*%s\s*\\)\\([a-zA-Z0-9']+\\)")

(defconst haskell-control-char-format " %s ")

(defconst haskell-control-characters
  '(("Subscript" "__" sub)
    ("Superscript" "^^" sup)))

(defconst haskell-control-region-format-regexp "\\(\s*%s\{\\)\\([^}]*\\)\\(\}\s*\\)")

(defconst haskell-control-regions
  '(("Subscript" "," "" sub)
    ("Superscript" "^" "" sup)
    ("Bold" "BOLD" "" bold)
...
(provide 'haskell-unicode-tokens)

;;; haskell-unicode-tokens.el ends here
