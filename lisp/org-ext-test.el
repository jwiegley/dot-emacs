...
(ert-deftest org-ext-story-d-stored-link-round-trips-or-does-nothing ()
  (with-temp-buffer
    ...
    (insert "* TODO Entry\n")
    (goto-char (point-min))
    (let ((org-stored-links '(("https://example.test/a" "Example"))))
      (cl-letf (((symbol-function 'org-ext-todoize) #'ignore))
        (org-ext-set-stored-link))
      (should (equal (org-entry-get nil "URL")
                     "[[https://example.test/a][Example]]"))
      (should (member "LINK" (org-get-tags))))
    (let ((original (buffer-string))
          (org-stored-links nil))
      (should-error (org-ext-set-stored-link) :type 'user-error)
      (should (equal original (buffer-string))))))

(ert-deftest org-ext-story-d-date-comes-from-valid-filename ()
  (with-temp-buffer
    (insert "#+date: [1999-01-01 Fri]\n")
    (setq buffer-file-name "/tmp/20240229-note.org")
    (org-ext-update-date-field)
    (should (string-match-p "2024-02-29" (buffer-string)))
    (erase-buffer)
    (insert "#+date: KEEP\n")
    (setq buffer-file-name "/tmp/20240230-invalid.org")
    (org-ext-update-date-field)
    (should (equal (buffer-string) "#+date: KEEP\n"))))

(ert-deftest org-ext-story-d-todoize-prefix-controls-id-force ()
  (with-temp-buffer
    (org-mode)
    (insert "* TODO Entry\n:PROPERTIES:\n:ID: old\n:END:\n")
    (goto-char (point-min))
    (let ((org-capture-before-finalize-hook nil))
      (cl-letf (((symbol-function 'org-id-get-create)
        ...
        (org-ext-todoize '(4))
        (should (equal (org-entry-get nil "ID") "new"))
        (org-entry-put nil "ID" "old")
        (org-ext-todoize nil)
        (should (equal (org-entry-get nil "ID") "old"))))))

(ert-deftest org-ext-story-d-inactive-time-falls-back-without-debugger ()
  (with-temp-buffer
    (org-mode)
    (insert "* DONE Entry\n")
    (goto-char (point-min))
    (let (arguments)
      (cl-letf (((symbol-function 'float-time)
                ...
                ((symbol-function 'debug)
                 (lambda (&rest _) (ert-fail "debugger entered"))))
        (should (= (org-ext-get-inactive-time) 42.0))
        (org-entry-put nil "CREATED" "[2024-01-02 Tue]")
        (should (= (org-ext-get-inactive-time) 1.0)))
      (should (= (length arguments) 2))
      (should (car arguments))
      (should-not (cadr arguments)))))

(ert-deftest org-ext-story-d-property-inheritance-preserves-all-forms ()
  (dolist (case '((nil . ("WITH"))
                  (t . t)
                  (("OWNER") . ("OWNER" "WITH"))
                  ("^OWNER$" . regexp)))
    (let ((org-use-property-inheritance (car case))
          observed)
      (cl-letf (((symbol-function 'org-tags-view)
                 (lambda (&rest _)
                   (setq observed org-use-property-inheritance))))
        (org-ext-with-property-search "OWNER" "John"))
      (if (eq (cdr case) 'regexp)
          (progn
            (should (string-match-p observed "OWNER"))
            (should (string-match-p observed "WITH"))
            (should-not (string-match-p observed "OTHER")))
        (should (equal observed (cdr case)))))))

(ert-deftest org-ext-story-d-parent-priority-is-character-or-nil ()
  (with-temp-buffer
    (org-mode)
    (insert "* TODO Top\n")
    (goto-char (point-min))
    (should-not (org-ext-parent-priority))
    (erase-buffer)
    (insert "* TODO Parent\n** TODO Child\n")
    (goto-char (point-max))
    (org-back-to-heading t)
    (should-not (org-ext-parent-priority))
    (erase-buffer)
    (insert "* TODO [#A] Parent\n** TODO Child\n")
    (goto-char (point-max))
    (org-back-to-heading t)
    (should (eq (org-ext-parent-priority) ?A))))

(ert-deftest org-ext-story-d-ancestor-keywords-exclude-current-heading ()
  (with-temp-buffer
    (org-mode)
    (insert "* TODO Top\n")
    (goto-char (point-min))
    (should-not (org-ext-ancestor-keywords))
    (erase-buffer)
    (insert "* TODO Parent\n** NEXT Child\n")
    (goto-char (point-max))
    (org-back-to-heading t)
    (should (equal (org-ext-ancestor-keywords) '("TODO")))))
