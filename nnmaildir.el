	...
	(when (or isnew nattr)
	  (dolist (file  (funcall ls ndir nil "\\`[^.]" 'nosort))
	    ...
	    (and (time-less-p (nth 5 (file-attributes x)) (current-time))
		 (rename-file x (if (string-match ":2," file)
				    (concat cdir file)
				  (concat cdir file ":2,")))))
  ...
  (let ((group (nnmaildir--prepare server gname))
	...
	(setq file (concat file "M" (number-to-string (caddr time)))))
      (setq file (concat file nnmaildir--delivery-pid)
	    ...
	    tmpfile (concat (nnmaildir--tmp dir) file)
	    curfile (if (string-match ":2," file)
			(concat (nnmaildir--cur dir) file)
		      (concat (nnmaildir--cur dir) file ":2,")))
      (when (file-exists-p tmpfile)
	(setf (nnmaildir--srv-error nnmaildir--cur-server)
	      (concat "File exists: " tmpfile))
	(throw 'return nil))
	...
	(error
	 ...
	 (throw 'return nil)))
	  ...
	  (setq article (make-nnmaildir--art :prefix file :suffix ""))
