;;; gitsum-new.el --- a pcl-cvs like interface for managing Git patches
...
;; Maintainer: Of gitsum.el fork: Christian Neukirchen <chneukirchen@gmail.com>
;; Maintainer: Of gitum.el: Christian Neukirchen <chneukirchen@gmail.com>
;; Maintainer: Of gitum-new.el: John Wiegley <johnw@newartisans.com>
;; Keywords: completion convenience tools vc
;; Version: 2.00-pre1
;; location: http://www.newartisans.com/software/emacs.html
;;           http://chneukirchen.org/repos/gitsum
...
;; Load this file and run M-x gitsum.  This will display a pcl-cvs like buffer
;; showing your modified, added, deleted and unknown files.  RET on a file to
;; reveal its specific changes; RET on a directory to reveal the changes to
;; all its files.

;; Type 'm' to register a file or a hunk in the index cache.  This causes the
;; related object to be displayed in bold.  Type 'c' to store all such staged
;; changes as a commit.  Type 'p' to store the changes as a Stacked Git patch.

;; Note that unlike darcsum.el, only those changes which are bold will be
;; commited when you press 'c' (darcsum had a default behavior, when no
;; changes or filed were marked, to commit which patches were visible in the
;; status buffer).  Because of the way Git uses the index cache, I've decided
;; to change this behavior, to make the process of building up a commit
;; slightly more manual.

;; The advantage of relying on the index cache this way is three-fold:

;;  1. It causes you to look at, and review, each of your changes before
;;     actually committing.  You'd be surprised at how illimunating it
;;     can be simply to look at a diff before you check in your code.

;;  2. If you register changes to be checked in, but then your Emacs
;;     crashes and you come back to gitsum later, the same changes will
;;     be seen as registered that were before.  This is because gitsum
;;     relies on the state of the index cache to determine what will be
;;     committed.

;;  3. If you manipulate the index cache outside of gitsum, just type
;;     'g' in the gitsum buffer to refresh its contents, which will now
;;     include your externally added changes as bolded items.

;;; Commentary:

;; Git is a content archiving system which stores trees of file contents in
;; the form of "commits".  Each commit has an associated "check-in" date, the
;; name of who created it, and a pointer to a tree of objects which ultimately
;; reference the contents, or "blobs", stored by that commit.

;; To view the contents of a commit, you need only run git-stash (if you have
;; changes in your working tree that need to be temporarily preserved), and
;; git-checkout.  The filesystem contents within your project will now
;; represent the contents of the tree registered by that commit.

;; Because one tends to commit the same files over and over again, it creates
;; the notion of a "history", representing the changes that occur over time
;; among those files.  The tool for viewing this history on the comand-line is
;; git-diff, once you know the hash ids of the two (or more) commits in
;; question whose difference you'd like to view.

;; But working with a diff like this is far from convenient.  Tools like gitk
;; make change browsing much more interesting, but I still find myself longing
;; for a nice Emacs interface for doing the same thing.  Enter gitsum-new.el.

;; The job of gitsum.el is to let you visualize a patch as a series of
;; "current" changes applicable to a specified "base".  Between these "current"
;; and "base" content trees exists the index cache, which is used to stage
;; intended changes between.  Thus the typical picture looks like this:

;;   [Current]       The working set reflected in the filesystem, and
;;      |            against which changes may be made.
;;      |
;;      |            Default: A checked out copy of "Base".
;;      v
;;   [Index Cache]   Changes that have been staged for commiting
;;      |              (i.e., the current "patch in progress")
;;      v
;;   [Base]          The commit our patch will be applied against.
;;                   
;;                   Default: The current branch's HEAD.

;; One aspect of gitsum-new.el's flexibilty is that both Current and Base can
;; set to a different commit id at any time.  If there are presently changes
;; in either the working tree or the index cache, the user's working set must
;; be stashed first using C-c C-s (it can be restored at any time using C-c
;; C-r).
