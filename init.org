...
(use-package gptel
  ...
  ;; (add-hook 'gptel-augment-handler-functions #'my/apply-gptel-preset)

  (add-hook 'gptel-post-response-functions #'my/gptel-next-prompt)
  ...
  (use-package gptel-openai
    ...
    (gptel-make-openai "ChatGPT"
      :key 'gptel-api-key
      ;; :stream t
      :models '(gpt-4.1 gpt-4o o1 gpt-4.5-preview))
    ...
    ;;   :protocol "http"
    ;;   ;; :stream t
    ;;   :request-params '(:options (:num_ctx 65536))
    ;;   :models '(
  ...
  (use-package gptel-openai-extras
    ...
    (gptel-make-perplexity "Perplexity"
      ...
      :models '(sonar-pro
                ...
                sonar-deep-research
                r1-1776)
      :request-params `(:web_search_options
                        (:search_context_size
                         "high"
                         :user_location
                         (:latitude
...
(use-package gptel-extra
  ...
  :bind
  (:map gptel-mode-map
        ("C-c C-t" . gptel-extra-clear-buffer))
  :config
...
(use-package whisper
  :no-require t
  :preface
  (defun whisper-transcribe-file (filepath)
    "Transcribe audio FILEPATH using MLX Whisper and insert results"
    (interactive "fSelect audio file: ")
    (let ((output-buffer (get-buffer-create "*Whisper Output*"))
          (model "mlx-community/whisper-large-v3-mlx"))
      (async-shell-command
       (format "mlx_whisper '%s' --model '%s' -f txt" filepath model)
       output-buffer)
      (display-buffer output-buffer)))

  (defvar whisper-recording-process nil
    "Process handle for active audio recording")

  (defvar whisper-temp-file (format "/tmp/emacs-whisper-%s.wav" (user-uid))
    "Temporary storage for audio recordings")

  (defun whisper-toggle-recording ()
    "Start/stop voice recording using FFmpeg"
    (interactive)
    (if (process-live-p whisper-recording-process)
        (whisper-stop-recording)
      (whisper-start-recording)))

  (defun whisper-start-recording ()
    "Initialize audio capture via AVFoundation"
    (setq whisper-recording-process
          (start-process "whisper-rec" nil "ffmpeg"
                         "-f" "avfoundation"
                         "-i" ":1"
                         "-t" "300"  ;; Max 5-minute recordings
                         "-y" whisper-temp-file))
    (message "Recording started..."))

  (defun whisper-stop-recording ()
    "Terminate recording and initiate transcription"
    (when (process-live-p whisper-recording-process)
      (delete-process whisper-recording-process))
    (whisper-transcribe-and-insert))

  (defun whisper-transcribe-and-insert ()
    "Process recorded audio and insert transcription"
    (if (file-exists-p whisper-temp-file)
        (progn
          (message "Transcribing...")
          (shell-command-to-string
           (format "mlx_whisper '%s' --model 'mlx-community/whisper-medium-mlx' -f txt -o -"
                   whisper-temp-file))
          (insert (with-current-buffer "*Shell Command Output*"
                    (buffer-string)))
          (delete-file whisper-temp-file))
      (error "No audio data captured")))
  :config
  (global-set-key (kbd "C-c w r") 'whisper-toggle-recording))
#+end_src
...
(use-package haskell-eglot
  ...
  :after (eglot)
  :config
  (dolist (where '((python-mode-hook   . python-mode-map)
                   (elpy-mode-hook     . elpy-mode-map)
                   (haskell-mode-hook  . haskel-mode-map)))
    (add-hook (car where)
              `(lambda ()
                 (bind-key "M-n" #'flymake-goto-next-error ,(cdr where))
                 (bind-key "M-p" #'flymake-goto-prev-error ,(cdr where)))))
...
[PACKAGES]"
  ...
  (org-block-begin-line ((t (:inherit org-meta-line :extend t :background "#120022"))))
  :defines
...
(use-package vulpea-extra
  ...
  :hook
  (org-roam-db-autosync-mode . vulpea-extra-db-setup-dates)
  :config
  (vulpea-auto-update-install)
  (vulpea-extra-db-setup-dates)
...
(use-package org-roam-extra
  ...
  (org-roam-mode . org-roam-extra-paint-transcript)
  :config
