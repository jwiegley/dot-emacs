...
[PACKAGES]"
  ...
  (org-tag-persistent-alist
   '(("Call" . 99)
     ...
     ("Nasim" . 78)))
  (org-tag-faces
   '(("HOT"  . (:foreground "Red" :weight bold))
	  ...
	  (with-current-buffer buffer
	    (org-with-wide-buffer
	     ...
	     (unless (looking-at-p re)
	       (user-error "Invalid refile position: expected %s but looking at %s"
                           re (buffer-substring (point) (line-end-position)))))))))))
#+end_src
...
,#+title: ${title}\n")
  ...
  (defun my/org-read-date ()
    (with-temp-buffer
      (org-time-stamp nil)
      (buffer-string)))

  (defun my/org-date (&optional timestamp offset)
    (with-temp-buffer
