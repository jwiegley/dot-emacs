...
(use-package aider
  ...
  (aider-args
   `("-c" "~/.config/aider/config.yaml"
     "--model" ,(concat "openai/" (symbol-name gptel-model))
...
(use-package gptel-backends
  ...
  (setq
   ;; gptel-model 'hera/Qwen3-Coder-480B-A35B-Instruct
   gptel-model 'hera/Kimi-K2-Instruct
  ...
  (defun my/org-reviewed-today ()
    ...
    (if (ignore-errors
        ...
        (progn
          ...
          (org-schedule '(4))
          (org-deadline '(4)))
      (when (or (ignore-errors (org-get-todo-state))
                (save-excursion
                  (goto-char (point-min))
