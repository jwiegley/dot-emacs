...
(use-package gptel
  ...
  :preface
  (defun my-gptel-read-prompts (dir)
    (cl-loop
     for file in (directory-files dir t "\\.txt\\'")
     if (string-match "/\\([^/]+?\\)\\.txt\\'" file)
     collect (cons (intern (match-string 1 file))
                   (with-temp-buffer
  ...
  (defun my-gptel-update-prompts ()
    ...
    (setq gptel-directives
          (my-gptel-read-prompts "~/.emacs.d/prompts")))
  :config
    ...
    (gptel-make-ollama "Ollama"
      ...
      '(
        (HammerAI/midnight-miqu-70b-v1.5:latest
         :description "Model designed for roleplaying and storytelling"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (athene-v2:72b
         :description "Athene-V2 excels at code completion, math and log extraction"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (command-r:35b
         :description "Command-R is a 35B model with 128k context length from Cohere"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (deepseek-coder:33b
         :description "Mixture-of-Experts code language model, trained on 2 trillion tokens"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (deepseek-coder-v2:16b
         :description "Mixture-of-Experts code language model, trained on 6+ trillion tokens"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        ...
        (deepseek-r1:671b
         ...
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (deepseek-v3:671b
         :description "Strong MoE model with 37B activated for each token"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (dolphin3:latest
         :description "General purpose LM for coding, math, function calling, and general use"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (falcon3:10b
         :description "Efficient AI models under 10B performant in science, math, and coding"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (gemma2:27b
         ...
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (hermes3:405b
         :description "Generalist model with advanced agentic capabilities"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (llama3.1:405b
         ...
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (llama3.2-vision:90b
         :description "Llama 3.2 Vision instruction-tuned image reasoning generative model"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (llama3.3:70b
         :description "Llama 3.3 70B offers performance comparable to Llama 3.1 405B"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (mistral-large:123b
         :description "Large version of Mistral model"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (mistral-nemo:12b
         :description "LLM with 128k context, by Mistral AI in collaboration with NVIDIA"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (mistral-small:latest
         :description "Mistral Small 3 sets new benchmark in “small” LLMs category below 70B"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (mistral:7b
         ...
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (nemotron:70b
         :description "LLM customized by NVIDIA to improve the helpfulness of responses"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (olmo2:13b
         :description "Competitive with open-weight models on English academic benchmarks"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (openthinker:32b
         :description "Fine-tuned version of Qwen2.5-32B-Instruct on OpenThoughts-114k dataset"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (orca-mini:70b
        ...
        (phi4:latest
         ...
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (qwen2-math:72b
         :description "Qwen2-Math specialized math language models built upon the Qwen2 LLMs"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (qwen2.5-coder:32b
         :description "Code-Specific Qwen, for code generation, reasoning, and fixing"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (qwen2.5:14b
         :description "Qwen2.5 models pretrained on Alibaba's latest large-scale dataset"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (qwen2.5:32b
         :description "Qwen2.5 models pretrained on Alibaba's latest large-scale dataset"
        ...
        (hf.co/lmstudio-community/Qwen2.5-14B-Instruct-1M-GGUF:Q8_0
         :description "Qwen2.5 model supporting up to 1M token context"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (qwq:32b
         :description "QwQ is a research model focused on advancing AI reasoning capabilities"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (starcoder2:15b
         ...
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (tulu3:70b
         :description "Fine-tuned LLM llama-3.3, optimized for tool usage and multi-turn dialogue"
         :capabilities (media)
         :mime-types ("image/jpeg" "image/png" "text/plain"))
        (wizardcoder:33b
...
(use-package corsair
  :demand t
  :bind
  ("C-x C C" . corsair-open-chat-buffer)
  ("C-x C w" . corsair-accumulate-selected-text)
  :preface
...
[[https://github.com/natrys/whisper.el?tab=readme-ov-file][whisper.el]] is a speech-to-Text interface for Emacs using OpenAI’s [[https://github.com/openai/whisper][whisper
speech recognition model]]. For the inference engine it uses the awesome C/C++
port [[https://github.com/ggerganov/whisper.cpp][whisper.cpp]] that can run on consumer grade CPU (without requiring a high
end GPU).

You can capture audio with your local input device (microphone) or choose a
media file on disk in your local language, and have the transcribed text
pasted to your Emacs buffer (optionally after translating to English). This
runs offline without having to use non-free cloud service for decent result
(though result quality of whisper varies widely depending on language, see
below).

#+begin_src emacs-lisp
(use-package whisper
  :bind ("C-c C-x w" . whisper-run)
  :config
...
(use-package org-ql
  ...
  (defun my/org-agenda-completion-hook ()
    ...
    (add-to-list 'completion-at-point-functions
                 'my/org-agenda-completions-at-point))

  (defun my/org-ql-find-refile-targets ()
    (interactive)
    (require 'org-ql)
    (require 'org-ql-find)
...
(use-package org-extra
  ...
  :bind (:map
         org-mode-map
         ("C-c M-w"     . org-extra-id-copy)
         ("C-c C-M-o"   . org-extra-open-all-links-in-subtree)
         ("C-c C-x B"   . org-extra-set-blocker-from-clipboard-id)
  ...
  ;; :hook
  ;; (org-agenda-finalize . org-extra-report-items-to-be-reviewed)
  :custom
  (org-extra-link-names
   (org-extra-read-names (org-file "kadena/team/202409042228-team.org")))
  :preface
...
(use-package org-quick-peek
  ...
  :config
  ;;; jww (2024-10-27): Fixing org-quick-peek creating invisible overlay when
  ;;; agenda filter is in place and the next item is hidden by the filter
...
jww (2025-03-10): This is currently fails to load.
...
(use-package org-project-capture
  ...
  (org-project-capture-projects-file "~/org/OSS.org")
  :config
...
(use-package org-roam-extra
  :after org-roam
  :demand t
  :bind* ("C-c M-m" . org-roam-extra-create-new)
  :bind
  (:map
   org-mode-map
   ("C-c C-x S" . org-roam-extra-sync)
   :map
   my-ctrl-c-n-map
   ("S" . org-roam-extra-import-fireflies)
   ("I" . org-roam-extra-node-insert-immediate)
   ("T" . org-roam-extra-revise-title)
   ("n" . org-roam-extra-create-new)
   ("W" . org-roam-extra-prepare-note)
   ("P" . org-roam-extra-meeting-audio))
  :hook
  (before-save . org-roam-extra-sort-file-properties)
  ;; (org-roam-file-setup . org-roam-extra-update-todo-tag)
  ;; (before-save . org-roam-extra-update-todo-tag)
  :config
  (cl-defmethod org-roam-node-slug ((node org-roam-node))
    "Return the slug of NODE."
    (org-roam-extra-title-slug (org-roam-node-title node))))
#+end_src
