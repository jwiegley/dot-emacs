...
:END:" :prepend t)
     ("s" "Slack" entry
      (file+headline "~/kadena/docs/kadena.org" "Pending")
      "* TODO %?
...
(use-package org-contacts
  :after org
  :custom
  (org-contacts-files '("~/doc/org/people.org")))
#+end_src
...
(use-package org-roam
  ...
  (org-roam-capture-templates
   '(("d" "default" plain "%?"
      ...
      :unnarrowed t)
     ("q" "quote" plain "%c\n\n─ %?"
      :target (file+head "quote/%<%Y%m%d%H%M>-${slug}.org"
                         "#+filetags: :quote:\n#+title: ${title}\n")
      :empty-lines-before 1)
     ("t" "thoughts" plain "%?"
      :target (file+head "thoughts/%<%Y%m%d%H%M>-${slug}.org"
                         "#+filetags: :thoughts:\n#+title: ${title}\n")
      :empty-lines-before 1)
     ("k" "kadena" plain "%?"
...
,#+filetags: :publish=newartisans:\n#+title: ${title}\n")
  ...
  (org-roam-file-exclude-regexp '("data/" "archive/" "\\<todo\\.org" "\\<people\\.org" "\\<index\\.org" "\\<OSS\\.org" "\\<habits\\.org"))
  :defines
  (org-roam-v2-ack)
  :functions
...
(use-package xeft
  ...
  (xeft-directory-filter 'my-xeft-directory-filter)
  (xeft-recursive t)
...
‘xeft-ignore-extension’."
  ...
  (defun my-xeft-directory-filter (dir)
    "Return nil if DIR should be ignored."
    (not (member (file-name-nondirectory dir)
                 '(".git" "data" "site" "_cache")))))
#+end_src
