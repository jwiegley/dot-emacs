...
(use-package gptel
  ...
  (defun my-gptel-read-prompts (dir)
    (cl-loop
     for file in (directory-files dir t "\\.\\(txt\\|md\\|org\\)\\'")
     if (string-match "/\\([^/]+?\\)\\.\\(txt\\|md\\|org\\)\\'" file)
  ...
  (defun my/apply-gptel-preset (_fsm)
    (if (= (point)
           (progn (gptel-beginning-of-response) (point)))
        (goto-char (point-min))
      (gptel-end-of-response))
    (when-let* (((looking-at "[[:blank:]\n]*@\\([[:word:]]+\\)[[:punct:]]*\\s-+"))
                (name (match-string 1))
                (preset (or (gptel-get-preset (intern name))
                            (gptel-get-preset name))))
      (delete-region (match-beginning 0) (match-end 0))
      (gptel--apply-preset (cons name preset)
                           (lambda (sym val)
                             (set (make-local-variable sym) val)))
      (message "Request sent with preset %s applied!"
               (propertize name 'face 'highlight))))
  :config
  ...
  (use-package gptel-openai
    ...
    ;;   :host "192.168.50.5:12345"
    ;;   :protocol "http"
    ;;   :stream t
    ;;   ;; :endpoint "/api/v0/chat/completions"
    ;;   ;; :request-params '(:options (:num_ctx 32767))
    ;;   :models '(
