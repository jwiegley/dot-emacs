...
(use-package llm-setup
  ...
  :commands (llm-setup-aider-model-name
             llm-setup-build-yaml
             llm-setup-status
             llm-setup-unload
...
(use-package aider
  ...
  (aider-args
   `("-c" "~/.config/aider/config.yaml"
     "--model" ,(llm-setup-aider-model-name)
...
Retired with the submodule-to-Nix migration: =erc-yank= remains excluded from
the normal Nix environment.  The former configuration is retained for
reference.
...
(use-package anvil
  ...
  (use-package anvil-ext
    :demand t)
  (setq anvil-optional-modules
        '(ide elisp sexp semantic sqlite pdf cron state shell-filter context))
  (anvil-enable)
  (use-package anvil-server-commands
    :config
    (setq anvil-server-autostart-on-request t)
    (anvil-server-start)))
#+end_src
...
(use-package haskell-setup
  :nix nil
  :commands (haskell-edit-reformat)
  :init
  (with-eval-after-load 'haskell-mode
    (keymap-set haskell-mode-map "C-c M-q" #'haskell-edit-reformat)))
#+end_src
...
(see =~/src/dot-emacs/lisp/docs/org-llm-wiki.md=).  Interactive and GPTel
integration remain enabled; the legacy =mcp-server-lib= adapter is disabled
with the retired MCP server configuration above.
...
(use-package org-wiki-gptel
  ...
  :custom
  (org-wiki-gptel-model 'hera/Qwen3.6-27B-Instruct)
  :config
