...
** Load Nix-managed packages into the macOS application

#+begin_src emacs-lisp
;; Nix-wrapped manual builds already provide their own package environment.
(when (and (eq system-type 'darwin)
           (not (string-prefix-p "/nix/store/"
                                 (file-truename invocation-directory))))
  (load (expand-file-name "emacs/nix-packages.el"
...
Mark buffer regions as pending an asynchronous result, with an
animated lighter, then atomically replace the placeholder when the
result arrives.  Used by =gptel-ext= for in-buffer LLM placeholders;
available for any other module that wants the same UX.

#+begin_src emacs-lisp
(use-package pending :defer t)
#+end_src
...
(use-package llm-setup
  :load-path "lisp/llm-setup"
  :defines (llm-setup-default-instance-name)
  :functions (llm-setup-gptel-backends)
  :commands (llm-setup-build-yaml
             llm-setup-status
             llm-setup-unload
             llm-setup-installed-models
...
(use-package aider
  ...
  (aider-args
   `("-c" "~/.config/aider/config.yaml"
     "--model" ,(format "openai/%s" llm-setup-default-instance-name)
     "--openai-api-base" "http://localhost:8000/v1"
     "--api-key" "openai=dummy-key"
...
(use-package dired-ext
  :bind ("C-c j" . dired-ext-open-group)

  :bind (:map dired-mode-map
              ("e"     . dired-ext-ediff-files)
              ("<tab>" . dired-ext-next-window))
  :hook
  (emacs-startup . dired-ext-two-pane))
#+end_src
...
(use-package docker
  ...
  (use-package docker-network :commands docker-networks)
...
(use-package gptel-backends
  ...
  (setq gptel-backend (gptel-backends-hermes)
        gptel-model 'hermes-agent))
#+end_src
...
(use-package llm-tool-collection
  ...
  (mapcar (apply-partially #'apply #'gptel-make-tool)
          (llm-tool-collection-get-tag 'introspection)))
#+end_src
  ...
  (setq Info-default-directory-list
        (mapcar
         ...
         (delq
          nil
          (append
           (parse-colon-path (getenv "INFOPATH"))
...
Disabled because the global =magit-todos-mode= scans the worktree on
every =magit-status= refresh, which dominated refresh latency on
=~/srp= (~10s).  Re-enable on demand with =M-x magit-todos-list= once
the package config below is restored.
...
Disabled because Forge injects =forge-insert-pullreqs=,
=forge-insert-issues=, and =forge-insert-assigned-pullreqs= into
=magit-status-sections-hook=, and the =gitea@gitea:= remotes used
locally don't fully resolve in the Forge adapter -- so every refresh
paid a network/DNS cost.  Re-enable when working on a repo with a
Forge-supported host (GitHub, GitLab, etc.).
...
#+begin_src emacs-lisp
(use-package mdformat
  :bind (:map markdown-mode-map
              ("C-c C-f" . mdformat-buffer)))
#+end_src
...
(use-package personal
  ...
  (("<C-M-backspace>" . backward-kill-sexp)
   ...
   ("C-x S"   . iTerm2-send-to-current-window)
...
(use-package tramp
  ...
  :config
  ;; Use the remote shell's own PATH (via ~/.profile) so executables
  ;; installed in places like ~/.nix-profile/bin are discoverable.
  (add-to-list 'tramp-remote-path 'tramp-own-remote-path)
  (add-to-list 'tramp-remote-path
               (expand-file-name "bin" (getenv "PROFILE_DIR")))

  (defun my-vulcan-file-handler (op &rest args)
    (let ((actual (replace-regexp-in-string
                   "^/vulcan:" "/ssh:vulcan|sudo:root@vulcan:" (car args))))
      (apply op (cons actual (cdr args)))))

  (add-to-list 'file-name-handler-alist
               '("\\`/vulcan:" . my-vulcan-file-handler))

  (defun my-andoria-file-handler (op &rest args)
    (let ((actual (replace-regexp-in-string
                   "^/pos:" "/ssh:andoria-08:" (car args))))
      (apply op (cons actual (cdr args)))))

  (add-to-list 'file-name-handler-alist
               '("\\`/pos:" . my-andoria-file-handler))

  (defun my-git-ai-file-handler (op &rest args)
    (let ((actual (replace-regexp-in-string
                   "^/git-ai:" "/ssh:git-ai:" (car args))))
      (apply op (cons actual (cdr args)))))

  (add-to-list 'file-name-handler-alist
               '("\\`/git-ai:" . my-git-ai-file-handler))

  (defun my-vps-file-handler (op &rest args)
    (let ((actual (replace-regexp-in-string
                   "^/vps:" "/ssh:vps:" (car args))))
      (apply op (cons actual (cdr args)))))

  (add-to-list 'file-name-handler-alist
               '("\\`/vps:" . my-vps-file-handler))

  (defun my-hera-file-handler (op &rest args)
    (let ((actual (replace-regexp-in-string
                   "^/hera:" "/ssh:hera:" (car args))))
      (apply op (cons actual (cdr args)))))

  (add-to-list 'file-name-handler-alist
               '("\\`/hera:" . my-hera-file-handler))
...
#+begin_src emacs-lisp
(use-package undercover :defer t)
#+end_src
...
(use-package whitespace
  ...
  (defun my-whitespace-after-cleanup ()
    (save-excursion
      (goto-char (point-min))
  ...
  :config
  (advice-add 'whitespace-cleanup :after #'my-whitespace-after-cleanup)
...
#+begin_src emacs-lisp
(use-package wikipedia-transient
  :bind ("C-c P" . wikipedia-transient))
#+end_src
...
(use-package cape
  ...
  (use-package cape-char
    :commands (cape-tex cape-sgml cape-rfc1345))
...
(use-package vertico
  ...
  (use-package vertico-flat
    :commands vertico-flat-mode)

  (use-package vertico-grid
    :commands vertico-grid-mode)

  (use-package vertico-reverse
    :commands vertico-reverse-mode)

  (use-package vertico-unobtrusive
    :commands vertico-unobtrusive-mode)
...
(use-package latex
  :nix auctex
  :mode ("\\.tex\\'" . LaTeX-mode)
  :bind (("s-َ"      . (lambda () (interactive) (insert "أ")))
...
Nael is a mode for editing Lean 4 programs.
...
(use-package proof-site
  ...
  (use-package coq-mode
    ...
    (defun my-coq-mode-hook-function ()
      ...
      (set (make-local-variable 'fill-nobreak-predicate)
           (lambda ()
             (pcase (get-text-property (point) 'face)
               ...
               (_ t)))))
    :config
    (add-hook 'coq-mode-hook #'my-coq-mode-hook-function))
...
(use-package gnus-start
  ...
  (gnus-parameters
   '(
     ...
     ("list/bahai/council"
      (to-list . "council.members@rbcca.org"))
     ("list/bahai/tarjuman"
      (to-list . "tarjuman@bahai-library-lists.org")
      (list-identifier . "\\[\\(tj\\|Tarjuman\\)\\]"))
     ("list/bahai/assembly"
      (gcc-self . t)
...
(use-package gnus-group
  ...
  (defvar gnus-group-name-map
    '(("nnimap+imap-assembly.vulcan.lan:INBOX" . "mail/assembly")
      ("nnimap+imap-bia.vulcan.lan:INBOX" . "mail/bia")
      ("nnimap+imap-rbcca.vulcan.lan:INBOX" . "mail/council"))
...
(use-package message
  ...
  (defun queue-message-if-not-connected ()
    (set (make-local-variable 'gnus-agent-queue-mail)
         (if (quickping smtpmail-smtp-server) t 'always)))
  :config
...
http://newartisans.com                          60E1 46C4 BD1A 7AC1 4BA2")
     ...
     ("BIA" "" "\"John Wiegley\" <john@bia.bahai.org>" "" nil "" "")
     ("Council" "" "\"John Wiegley\" <jwiegley@rbcca.org>" "" nil "" "")
...
[PACKAGES]"
  ...
  (org-todo-keywords
   '((sequence "DRAFT" "TODO(t@)" "DOING(s!)"
               "WAIT(w@)" "DEFER(r!)"
               "TASK(k@)" "HABIT(h)" "APPT(a)"
               "|" "DONE(d@)" "CANCELED(x@)" "PASS(p!)")
     (sequence "|" "SCRAP" "NOTE(n)" "QUOTE(q)" "PROMPT(P)" "LINK(l)")))
  (org-todo-keyword-faces
   `(
     ...
     ("QUOTE"    :foreground ,palette-purple-dark     :weight bold)
     ("PROMPT"   :foreground ,palette-purple-dark     :weight bold)
     ))
  (org-tag-faces
  ...
  (org-use-tag-inheritance
   '("Noon" "Day" "Eve" "Night" "Home" "Call" "Errand"
     "assembly" "council" "bia" "positron" "git-ai"))
  ...
  (defun my/org-auto-exclude-function (tag)
    (let ((hour (nth 2 (decode-time))))
      (and (cond
            ...
            ((string= tag "Nasim")
             ;; At home: defer to Home Assistant (`person.nasim_wiegley'),
             ;; matching how Node-RED on vulcan checks her presence.
             ;; Fail closed: exclude unless HA confirms she is home.
             ;; Away: fall back to the time heuristic.
             (if (org-ext-at-home-p)
                 (not (equal (org-home-assistant-state "person.nasim_wiegley")
                             "home"))
               (< hour 17)))
            ((string= tag "Eve")    (< hour 18))
            ((string= tag "Night")  (< hour 21))
            ((string= tag "Home")
             (not (org-ext-at-home-p)))
            ((string= tag "Net")
...
(use-package org-agenda-overlay
  ...
  (org-agenda-overlay-by-filetag
   `(("c2g"           face (:background ,palette-orange-darker  :foreground "#eeeeee"))
     ...
     ("family"        face (:background ,palette-yellow-darker  :foreground "#eeeeee"))
     ...
     ("git-ai"        face (:background ,palette-fuschia-darker :foreground "#eeeeee"))
     ;; drafts
     ("journal"       face (:background ,palette-fuschia        :foreground "#ffffff"))
     ))
  (org-agenda-overlay-by-olp
   `(;; Headings in todo.org
     ...
     (("Health")      face (:background ,palette-blue-darker    :foreground "#eeeeee"))
     ;; indigo
     (("Computer")    face (:background ,palette-indigo-darker  :foreground "#eeeeee"))
     ;; purple
     (("Open Source") face (:background ,palette-purple-darker  :foreground "#eeeeee"))))
  (org-agenda-overlay-by-category
   `(;; Bahá’í categories
     ("Council"   face (:background ,palette-green-darker   :foreground "#eeeeee"))
     ;; Family categories
     ("Nasim"     face (:background ,palette-yellow-darker  :foreground "#eeeeee"))
     ;; Personal categories
     ("Computer"  face (:background ,palette-indigo-darker  :foreground "#eeeeee"))
     ("AI"        face (:background ,palette-indigo-darker  :foreground "#eeeeee"))
     ("Nix"       face (:background ,palette-indigo-darker  :foreground "#eeeeee"))
     ("NixOS"     face (:background ,palette-indigo-darker  :foreground "#eeeeee"))
     ("Backups"   face (:background ,palette-indigo-darker  :foreground "#eeeeee"))
     ("Network"   face (:background ,palette-indigo-darker  :foreground "#eeeeee"))
     ("category"  face (:background ,palette-purple-darker  :foreground "#eeeeee"))
     ("gptel"     face (:background ,palette-purple-darker  :foreground "#eeeeee"))
     ("Emacs"     face (:background ,palette-purple-darker  :foreground "#eeeeee"))
     ("Ledger"    face (:background ,palette-purple-darker  :foreground "#eeeeee"))
     ("org-mode"  face (:background ,palette-purple-darker  :foreground "#eeeeee"))
     ;; Work categories
     ("Ares"      face (:background ,palette-fuschia-darker :foreground "#eeeeee"))
     ("Tron"      face (:background ,palette-fuschia-darker :foreground "#eeeeee"))
     ("Ingest"    face (:background ,palette-fuschia-darker :foreground "#eeeeee"))
     ("Positron"  face (:background ,palette-fuschia-darker :foreground "#eeeeee")))))
#+end_src
...
(use-package org-ql-ext
  ...
  (("M-g O" . org-ql-ext-find-refile-targets)
   :map
   org-mode-map
   ("C-c C-x X" . org-ql-ext-export-columnview))
  :config
  (eval-when-compile
    (require 'vertico-multiform))
  (add-to-list 'vertico-multiform-commands
...
This code will perform a cosine-similarity vector search against a set of
embeddings of my Org-mode entries in PostgreSQL using pgvector. This relies on
the =org db store= functionality in my =org-jw= Haskell project, and depends to a
rather high degree on following the precise structural conventions I use for
my Org-mode data.

#+begin_src emacs-lisp
(use-package org-ql-semantic
  :after (org)
  :commands (org-ql-semantic-search)
  :bind
  ("M-s s" . org-ql-semantic-search)
  :custom
  ;; (org-ql-semantic-org-executable
  ;;  "~/src/org-jw/dist-newstyle/build/aarch64-osx/ghc-9.10.3/org-jw-0.0.1/x/org/build/org/org")
  (org-ql-semantic-config-file "~/org/org.yaml")
  (org-ql-semantic-base-url "http://localhost:8080")
  (org-ql-semantic-model "bge-m3")
  (org-ql-semantic-api-key "unused")
  (org-ql-semantic-default-limit 2000)
  (org-ql-semantic-dimensions 1024)
  (org-ql-semantic-max-distance 0.35)
  :config
  (setq org-ql-semantic-db-url
...
(use-package org-ext
  ...
  :bind (:map
         ...
         ("C-c C-x E"     . org-ext-copy-subtree-as-markdown)
         ("C-c C-x P"     . org-ext-set-contact)
         ("C-c C-x V"     . org-ext-set-verb)
         ...
         ("C-c C-x C-l"   . org-ext-set-stored-link)
         ("C-c h"         . org-ext-insert-keyword-heading)
         ("C-c C-x m"     . org-ext-insert-message-link)
         ("C-c C-x s"     . org-ext-capture-link-to-entry)
         ("C-c C-x u"     . org-ext-todoize)
         ("C-c C-x C-,"   . org-ext-insert-structure-template-and-yank)
         ("C-c C-x C-M-y" . org-ext-yank-link)
         ("C-c C-x C-k"   . org-ext-switch-todo-task)
         ...
         ("K" . org-ext-agenda-switch-todo-task)
         ("C" . org-ext-agenda-set-category)
         ("P" . org-ext-agenda-set-contact)
         ("V" . org-ext-agenda-set-verb))
  :hook
...
#+begin_src emacs-lisp
(use-package org-home-assistant
  :commands (org-home-assistant-state
             org-home-assistant-clear-cache)
  :custom
  (org-home-assistant-base-url "https://hass.vulcan.lan")
  (org-home-assistant-token
   (lambda () (lookup-password "hass.vulcan.lan" "johnw" 443))))
#+end_src
...
(use-package org-capture-ext
  ...
  (org-capture-mode . org-ext-cleanup-whitespace)
...
(use-package org-drafts
  ...
  :bind (:map
         org-mode-map
         ("C-c H" . org-drafts-act-on-region))
  :config
  (org-drafts-install))
#+end_src
...
#+begin_src emacs-lisp
(use-package org-drafts-ext
  :after (org-drafts)
  :demand t
  :bind (:map
         org-mode-map
         ("C-c C-x y"   . org-drafts-ext-paste-prompt)
         ("C-c C-x C-y" . org-drafts-ext-paste-from-markdown))
  :config
...
(use-package org-contacts
  ...
  (org-contacts-capf-completing nil)
  :config
...
(use-package org-crypt
  ...
  ;; `org-encrypt-entries' uses `org-scan-tags', which walks the
  ;; buffer with `org-element-cache-map'.  Encrypting an entry
  ;; modifies the buffer, which invalidates the cache iterator and
  ;; trips an "Invalid search bound (wrong side of point)" error in
  ;; the cache.  Disable the element cache for the duration of the
  ;; scan -- the cache will rebuild after the save completes.
  (advice-add 'org-encrypt-entries :around
...
(use-package org-extra-emphasis
  ...
  :custom-face
  (org-extra-emphasis-01
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-yellow :foreground "black"))))
  (org-extra-emphasis-03
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-red :foreground "black"))))
  (org-extra-emphasis-04
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-orange :foreground "black"))))
  (org-extra-emphasis-05
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-yellow-green :foreground "black"))))
  (org-extra-emphasis-06
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-green :foreground "black"))))
  (org-extra-emphasis-07
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-teal :foreground "black"))))
  (org-extra-emphasis-08
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-cyan :foreground "black"))))
  (org-extra-emphasis-09
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-slate-blue :foreground "black"))))
  (org-extra-emphasis-10
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-blue :foreground "black"))))
  (org-extra-emphasis-11
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-indigo :foreground "black"))))
  (org-extra-emphasis-12
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-purple :foreground "black"))))
  (org-extra-emphasis-13
   ((t (:inherit org-extra-emphasis :bold t
                 :background ,palette-fuschia :foreground "black")))))
#+end_src
...
(use-package org-hash
  ...
  :diminish org-hash-mode
  :hook (org-mode . org-hash-mode)
  :custom
  (org-hash-recursive nil)
  (org-hash-update-on-save 'git)
  :config
  (setq safe-local-variable-values '((org-hash-update-on-save . nil))))
#+end_src
...
(use-package org-remark
  ...
  :init
  (use-package org-remark-line
    :bind (:map my-ctrl-c-n-map
...
#+begin_src emacs-lisp
(use-package org-review-ext
  :after (org-review)
  :demand t
  :bind
  (:map
   my-ctrl-c-n-map
   ("R" . org-review-ext-reviewed-today)
   ("r" . org-review-ext-insert-last-and-set-next-review)
   :map
   org-agenda-mode-map
   :prefix-map
   my/org-review-map
   ...
   ("R" . org-review-ext-reviewed-today)
   ("r" . org-review-ext-insert-last-and-set-next-review)
   ("d" . org-review-ext-next-day)
   ("D" . org-review-ext-next-three-days)
   ("w" . org-review-ext-next-week)
   ("2" . org-review-ext-next-two-days)
   ("@" . org-review-ext-next-two-weeks)
   ("3" . org-review-ext-next-three-days)
   ("#" . org-review-ext-next-three-weeks)
   ("4" . org-review-ext-next-four-days)
   ("$" . org-review-ext-next-four-weeks)
   ("5" . org-review-ext-next-five-days)
   ("%" . org-review-ext-next-five-weeks)
   ("6" . org-review-ext-next-six-days)
   ("^" . org-review-ext-next-six-weeks)
   ("m" . org-review-ext-next-month)
   ("M" . org-review-ext-next-two-months)
   ("t" . org-review-ext-next-two-months)
   ("q" . org-review-ext-next-quarter)
   ("Q" . org-review-ext-next-quarter)
   ("h" . org-review-ext-next-half-year)
   ("y" . org-review-ext-next-year)
   ("Y" . org-review-ext-next-two-years)
   )
  :config
  (add-hook 'org-capture-before-finalize-hook #'org-review-ext-reviewed-today)
  (add-hook 'org-after-todo-state-change-hook #'org-review-ext-reviewed-today)

  (advice-add 'org-review-insert-last-review
              :after #'org-review-ext-increment-count))
#+end_src

**** org-review-drafts

#+begin_src emacs-lisp
(use-package org-review-drafts
  :no-require t
  :after (org-drafts)
  :hook
  (org-drafts-after-state-change-function . org-review-ext-reviewed-today))
#+end_src
...
#+begin_src emacs-lisp
(use-package org-obsidian
  :no-require t
  :after (org)
  :config
  (org-add-link-type "obsidian" 'org-obsidian-open)

  (defsubst org-obsidian-open (record-location)
    (browse-url (concat "obsidian:" record-location))))
#+end_src
...
(use-package org-roam-dailies
  :after org-roam
  :demand t
  :bind* (("M-M" . org-roam-dailies-capture-today)
          ("H-m" . org-roam-dailies-capture-today))
  :bind
  (:map
   my-ctrl-c-n-map
...
Read-only spike of the Org-native LLM Wiki
(see =~/src/dot-emacs/lisp/docs/org-llm-wiki.md=).  Tools register on
the shared "default" endpoint once per Emacs lifetime; the MCP client
connects via emacs-mcp-stdio.sh with --server-id=default and no
init-function, so concurrent Claude Code sessions don't race the
registration.
...
(use-package org-wiki-commands
  :after (org-wiki)
  :commands (org-wiki-find org-wiki-search
             org-wiki-backlinks org-wiki-show-metadata)
  :bind ("C-c w w" . org-wiki-command-map))
...
(use-package org-wiki-gptel
  :after (gptel org-wiki)
  :demand t                      ; load eagerly once gptel+org-wiki are up so
                                 ; the 'org-wiki preset and wiki_* tools exist
                                 ; for @org-wiki and ob-gptel without first
                                 ; running a command
  :commands (org-wiki-ask org-wiki-chat)
  :config
  (with-eval-after-load 'org-wiki-commands
    (define-key org-wiki-command-map (kbd "a") #'org-wiki-ask)
    (define-key org-wiki-command-map (kbd "c") #'org-wiki-chat)))
#+end_src

*** org-agent-deck

#+begin_src emacs-lisp
(use-package org-agent-deck
  :after org
  :commands (org-agent-deck-send-entry
