...
(use-package gptel
  ...
  (defvar my/gptel-session-id nil)

  (defun my/gptel-set-session-id ()
    (require 'uuidgen)
    (setq-local my/gptel-session-id (uuidgen-1)))

  (defun my/gptel-add-litellm-session ()
    (when (and my/gptel-session-id
               (string= (gptel-backend-name gptel-backend) "LiteLLM"))
      (setq-local gptel--request-params
                  (gptel--merge-plists gptel--request-params
                                       (list :litellm_session
                                             my/gptel-session-id)))))

  (defun my/gptel-add-session-header (old-func fsm)
    (let* ((info (gptel-fsm-info fsm))
           (session-id
            (with-current-buffer (plist-get info :buffer)
              my/gptel-session-id)))
      (if session-id
          (let ((params (get gptel-model :request-params)))
            (unwind-protect
                (progn
                  (put gptel-model :request-params
                       (append (list :litellm_session_id session-id)
                               params))
                  (funcall old-func fsm))
              (put gptel-model :request-params params)))
        (funcall old-func fsm))))
  :config
  ;; These are defvars
  ...
  (add-hook 'gptel-mode-hook #'my/gptel-set-session-id)
  ;; (add-hook 'gptel-prompt-transform-functions
  ;;           #'my/gptel-add-litellm-session)
  (advice-add 'gptel--realize-query :around #'my/gptel-add-session-header)
  )
#+end_src
