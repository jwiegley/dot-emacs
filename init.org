...
[PACKAGES]"
  ...
  (org-todo-keywords
   '((sequence "TODO(t@/!)" "DOING(s!/!)"
               ...
               "|" "DONE(d@/!)" "CANCELED(x@/!)")
     (sequence "HABIT(h@/!)" "VISIT(v@/!)" "|" "FINISHED(@/!)")
  ...
  (org-todo-keyword-faces
   `(
     ...
     ("VISIT"    :foreground ,palette-purple          :weight bold)
...
(use-package org-quick-peek
  ...
  ;;; agenda filter is in place and the next item is hidden by the filter Bug
  ;;; report: yantar92 [Github] issue#9 The overlay is not always visible when
  ;;; org-agenda-filter is in place
...
#+begin_src emacs-lisp
(use-package org-refile-context
  :after org
  :bind (:map org-mode-map ("C-c C-x R" . org-refile-context-reverse))
  :config
  (org-refile-context-install))
#+end_src
...
(use-package org-review
  ...
  (defun my-org-reviewed-today ()
    ...
    (when (or (ignore-errors (org-entry-is-todo-p))
              (save-excursion
                (goto-char (point-min))
                (looking-at "^\\(TODO\\|TASK\\|HABIT\\|PROJECT\\|VISIT\\) ")))
      (save-excursion
