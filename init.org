...
(use-package gptel
  ...
  (gptel-model "gpt-4")
  :config
  (setq gptel-api-key (auth-source-pass-get 'secret "chat.openai.com")))
#+end_src
...
(use-package org
  ...
  ;; (org-mode . org-babel-result-hide-spec)
  ;; (org-mode . org-babel-hide-all-hashes)
...
[PACKAGES]"
  ...
  (org-todo-keywords
   '((sequence "TODO(t)" "STARTED(s)" "WAITING(w@/!)"
               ...
               "|" "DONE(d@)" "CANCELED(x@)")
     (sequence "PROJECT(p)" "EPIC(e)" "STORY(s)" "|" "DONE(d@)" "CANCELED(x@)")
     (sequence "CATEGORY" "|")
...
"))
  ...
  (defun my-insert-copied-slack (string)
    (with-temp-buffer
      (insert string)
      (goto-char (point-min))
      (whitespace-cleanup)
      (while (re-search-forward "\\[\\[.*\\.png\\]\\] +\\s-*\n+" nil t)
        (delete-region (match-beginning 0) (match-end 0)))
      (goto-char (point-min))
...
(use-package org-agenda
  ...
  (org-agenda-custom-commands
   '(("A" "Priority #A tasks" agenda ""
     ...
     ("D" "Deadlined tasks" tags "TODO<>\"\"&TODO<>{DONE\\|CANCELED\\|NOTE\\|LINK\\|PROJECT}"
      ((org-agenda-overriding-header "Deadlined tasks: ")
       (org-agenda-skip-function
        '(org-agenda-skip-entry-if 'notdeadline))
       (org-agenda-sorting-strategy
        '(category-up))))
     ("S" "Scheduled tasks" tags "TODO<>\"\"&TODO<>{APPT\\|DONE\\|CANCELED\\|NOTE\\|LINK\\|PROJECT}&STYLE<>\"habit\""
      ((org-agenda-overriding-header "Scheduled tasks: ")
       (org-agenda-skip-function
        '(org-agenda-skip-entry-if 'notscheduled))
       (org-agenda-sorting-strategy
        '(category-up))))
     ("o" "Unscheduled open source tasks" tags "TODO<>\"\"&TODO<>{DONE\\|CANCELED\\|NOTE\\|LINK\\|PROJECT\\|CATEGORY}"
      ((org-agenda-overriding-header "Unscheduled Open Source tasks: ")
       ...
       (org-agenda-files
        '("~/org/area/OSS.org"))))
     ("u" "Unscheduled tasks" tags "TODO<>\"\"&TODO<>{DONE\\|CANCELED\\|NOTE\\|LINK\\|PROJECT\\|CATEGORY\\|DEFERRED\\|SOMEDAY}"
     ...
     ("Y" "Someday tasks" tags "TODO=\"SOMEDAY\""
      ((org-agenda-overriding-header "Someday tasks:")
       (org-agenda-sorting-strategy
        '(user-defined-up))
       (org-agenda-prefix-format "%-11c%5(org-todo-age) ")))
     ("I" "Impactful tasks" tags "Impact&TODO<>\"\"&TODO<>{DONE\\|CANCELED\\|NOTE\\|LINK\\|PROJECT\\|CATEGORY\\|DEFERRED\\|SOMEDAY}"
      ((org-agenda-overriding-header "Impactful tasks:")
       (org-agenda-sorting-strategy
        '(todo-state-up priority-down user-defined-up))
       (org-agenda-prefix-format "%-11c%5(org-todo-age) ")))
     ("w" "Unscheduled work tasks" tags "TODO<>\"\"&TODO<>{DONE\\|DEFERRED\\|CANCELED\\|NOTE\\|LINK\\|PROJECT\\|CATEGORY}"
     ...
     ("L" "Links" tags "TODO=\"LINK\""
      ((org-agenda-overriding-header "Links")))))
    ...
    (let ((notes
      ...
      (when notes
        (with-current-buffer (find-file-noselect "~/org/area/todo.org")
          (save-excursion
            (goto-char (point-min))
            (re-search-forward "^\\* CATEGORY Inbox$")
...
(use-package org-capture
  ...
  (org-capture-templates
   '(("a" "Add Task" entry
      (file+headline "~/org/area/todo.org" "Inbox")
      "* TODO %?" :prepend t)
     ("n" "Note" entry
      (file+headline "~/org/area/todo.org" "Inbox")
      "* NOTE %?" :prepend t)
...
:END:" :prepend t)
     ("s" "Slack" entry
      (file+headline "~/org/area/kadena/kadena.org" "Pending")
      "* TODO %?
%(my-insert-copied-slack \"%c\")" :prepend t)))
  :preface
...
(use-package org-roam
  :demand t
  :after org
  :bind (("C-c n B" . my/org-roam-capture-inbox)
         ("C-c n I" . org-roam-node-insert-immediate)
         ("C-c n J" . add-journal-entry)
         ("C-c n L" . org-roam-buffer-toggle)
         ("C-c n T" . org-roam-revise-title)
         ("C-c n c" . org-roam-create-new)
  ...
  (org-roam-capture-templates
   '(("d" "default" plain "%?"
      ...
      :unnarrowed t)
     ("u" "unnamed" plain "%?"
      :target (file+head "inbox/%<%Y%m%d%H%M%S%N>.org"
                         "#+filetags: :roam:\n#+title: Unknown\n")
      :empty-lines-before 1
      :unnarrowed t)
     ("q" "quote" plain "%c\n\n─ %?"
...
,#+filetags: :publish=newartisans:\n#+title: ${title}\n")
  ...
  (cl-defun org-roam-create-new (&optional goto keys &key filter-fn templates info)
    (interactive "P")
    (let ((node (org-roam-node-create :title "Unknown")))
  ...
  (defcustom org-roam-revise-locations nil
    "A mapping of tags to relative folders in org-roam-directory.
The first tag to match is used."
    :group 'org-roam
    :type '(repeat (cons string string)))

  (defun org-roam-revise-mapping (tags)
    (let (found-match)
      (dolist (tag tags)
        (unless found-match
          (setq found-match
                (cdr (assoc tag org-roam-revise-locations)))))
      found-match))

  (defun org-roam-revise-title ()
    (interactive)
    (save-buffer)
    (org-roam-db-sync)
    (let ((title (org-roam-db--file-title))
      ...
      (let* ((new-name
             ...
             (new-name-nondirectory
              (and new-name (file-name-nondirectory new-name)))
        ...
        (when (and old-name-nondirectory new-name-nondirectory)
          (let ((old-stamp (and (string-match "^\\([0-9]\\{14\\}\\)"
                                              old-name-nondirectory)
                                (match-string 1 old-name-nondirectory)))
                (new-slug (and (string-match "^[0-9]\\{14\\}-\\(.+\\)"
                                             new-name-nondirectory)
                               (match-string 1 new-name-nondirectory))))
            (when (and old-stamp new-slug)
              (setq new-name
                    (expand-file-name (concat old-stamp "-" new-slug)
                                      (file-name-directory new-name)))))
          (setq new-name
                (expand-file-name
                 (file-name-nondirectory new-name)
                 (expand-file-name
                  (or (org-roam-revise-mapping tags)
                      (file-name-directory
...
‘xeft-ignore-extension’."
    (let ((relative-path (file-relative-name file org-roam-directory)))
      (and (file-regular-p file)
           ...
           (not (org-file-image-p file))
           (not (member relative-path
                        '("area/todo.org"
                          "area/habits.org"
                          "area/OSS.org"
                          "area/kadena/kadena.org"
                          "area/kadena/archive.org"
                          "area/assembly/assembly.org")))
           (not (let (is-match)
