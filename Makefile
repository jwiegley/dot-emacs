...
PROVERS=acl2 ccc coq demoisa hol98 isar lclam lego pgshell phox plastic
...
BYTECOMP = $(BATCHEMACS) -eval '(setq load-path (append (mapcar (lambda (d) (concat "${PWD}/" (symbol-name d))) (quote (${ELISP_DIRS}))) load-path))' -eval '(progn (require (quote bytecomp)) (setq byte-compile-warnings (remove (quote noruntime) byte-compile-warning-types)))' -f batch-byte-compile
