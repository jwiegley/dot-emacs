...
splitAtN n [] = []
splitAtN n xs
    = ys : (splitAtN n zs) where (ys, zs) = splitAt n xs
 
ffth :: [ComplexF] -> [ComplexF] -> [ComplexF]
 
ffth xs us
    | n > 1 =
        (ffmups (replikate fftEvn_2)
             (ffmus us (replikate fftOdd)))
    | n == 1 = xs
  where
      rvv = ffmus us (replikate fftOdd)
      rvv1 = replikate fftEvn_2
...
dfth
    :: [Int] -> [ComplexF] -> [ComplexF] -> [ComplexF]
...
dfth ((f : fs)) xs us
  ...
  where
      pfts
          =
              [(dftOfSection k) `times`
                   (us `toThe` k) | k <- [0 .. f - 1]]
      dftOfSection k
          = repl f (dfth fs (fsectionOfxs k) (us `toThe` f))
      fsectionOfxs k = everyNth f (drop k xs)
 
sft :: [ComplexF] -> [ComplexF]
...
sfth n xs us
    =
        [sum (xs `times` upowers) | upowers <- [us `toThe`
                                                    k | k <- [0 .. n - 1]]]
...
everyNth n
    =
        (map head) .
            ((takeWhile (/= [])) . (iterate (drop n)))
 
toThe :: [ComplexF] -> Int -> [ComplexF]
 
toThe rootsOfUnity k = everyNth k rootsOfUnity
