...
trD (d @ (Dec x)) =
 case mapD trE trP (map trD) bad bad bad x of
   ...
   HsFunBind loc matches -> Fun (name matches) (map g matches)
      where name ((HsMatch loc nm ps rhs ds): ms) = (show nm)
...
trE (e @ (Exp x)) = 
 case mapE trE trP (map trD) bad bad x of 
   HsId n                 -> Var (showId n)
   HsApp x y              -> App x y
   HsLit (HsInt n)        -> Const n
   HsInfixApp x op z      -> App (App (Var (showId op)) x) z
   HsNegApp x             -> App (Var "negate") x  
   ...
   HsParen x              -> x
   HsLeftSection x op     -> Abs[Pvar "zzz"] (App (App (Var (show op)) x) (Var "zzz"))
   HsRightSection op y    -> Abs[Pvar "zzz"] (App (App (Var (show op)) (Var "zzz")) y)
