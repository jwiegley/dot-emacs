-- $Id: Scope.hs,v 1.18 2001/08/21 00:13:36 hallgren Exp $
...
type Arity = Int
data KindOfName   = Var | Cons | TyVar | TyCons | Class | Mod | Sig
data KindOfTyCons = DataDef | NewTypeDef | TypeDef
...
locations k env =
  case k of
    ...
    other -> error "no locations available "


data Senv = S { classNames   :: [(HsName,SrcLoc,Arity,[HsName])]
...
instance Env Senv where 
 extClass   nm l a nms env = env {classNames   = (nm,l,a,nms): classNames   env}
 extTconstr nm l a syn env = env {tconstrNames = (nm,l,a,syn): tconstrNames env}
...
tconDefined  s env = (any (==s) $ map fst4 $ tconstrNames env) 
                     || (any (==s) [Qual (Module "Prelude") "()", Qual (Module "Prelude") "[]"])
...
classMethods nm env = fmap fth4 (List.find match (classNames env))
   where match (name,_,_,nms)  = name==nm
         match _               = False

synArity :: HsName -> Senv -> Maybe Int
synArity nm env = fmap thd4 (List.find match (tconstrNames env))
   where match (name,_,_,True) = name==nm
         match _               = False
   

cArity :: HsName -> Senv -> Maybe Int
cArity (UnQual ":") _ = Just 2
cArity (Qual (Module "Prelude") ":") _ = Just 2
cArity (UnQual "()") _ = Just 0
cArity (Qual (Module "Prelude") "()" ) _ = Just 0
cArity c env = fmap thd3 (List.find match (constrNames env))
...
names (Dec d) =
 case d of
  ...
  HsDataDecl s cntxt (c:args) constrs der -> 
    let f x xs = (dataName x) : xs
        g x xs = (recordName x) ++ xs
    in  env0 { tconstrNames   = [(getTyName c, s, length args, False)]
             , constrNames = foldr f [] constrs
             , varNames    = foldr g [] constrs
             }
  HsClassDecl s c tp ds ->
    let (con, args) = typePatToParts tp
        members  = sigNames $ allNames ds env0
    in  env0 { varNames    = members
  ...
  HsFunBind s ((HsMatch s2 nm ps rhs ds):_) -> env0 {varNames = [(nm,s2)]}
  HsPatBind s p rhs ds -> env0 {varNames = map (\nm -> (nm,s)) $ boundInP p []}
  HsPrimitiveTypeDecl s c nm ->  
     env0 { tconstrNames = [(nm,s,0,False)] }
  HsPrimitiveBind s nm typ ->
    env0 { varNames = [(nm,s)] }       
  other -> env0
...
collect_duplicate_info infof compare = 
  concat . (map infof) . (groupBy (lift compare)) . (List.sortBy compare)
...
nmConflict env = 
    let ts = tconstrNames env 
        ...
        sameName (tn,tloc,_,_) errs =
          case List.find (\(a,_,_,_) -> a==tn) cs of 
            Nothing           -> errs
...
chT loc (typ @ (Typ t)) env = 
  case t of 
   ...
   HsTyApp (Typ f) x -> synCheck (chT loc) loc env f x 1
   z -> accT (++) (scopT env (mapT (chT loc)  z)) []

synCheck chf loc env (HsTyCon c) arg n = 
    case synArity c env of
    Nothing -> chk loc (not (tconDefined c env)) (undefined_tcon c) ++
               chf arg env 
    Just m  -> chk loc (m/=n) (tysynonym_not_fully_applied c) ++ chf arg env
synCheck chf loc env (HsTyApp (Typ f) x) arg n =
    synCheck chf loc env f x (n+1) ++ chf arg env
synCheck chf loc env t arg n = chf (Typ t) env ++ chf arg env
...
run env prog = errF (envF env) where (envF, errF) = chDs TopDecl prog
