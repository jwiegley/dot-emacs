...
;;; MG: Slightly modified to run under Emacs Common Lisp


(defpackage "CLOSETTE"
#-emacs-cl
  (:shadow "DEFCLASS" "SLOT-VALUE" "SLOT-BOUNDP" "SLOT-MAKUNBOUND" 
	   "SLOT-EXISTS-P" "CLASS-OF" "CLASS-NAME" "FIND-CLASS"
	   "METHOD-QUALIFIERS" "DEFGENERIC" "DEFMETHOD" "ADD-METHOD" 
	   "REMOVE-METHOD" "FIND-METHOD" "ENSURE-GENERIC-FUNCTION" 
	   "PRINT-OBJECT"
...
;; MG: remove has some problems with :test-no
(defun not-my-eq (arg1 arg2)
  (not (eq arg1 arg2)))
...
(defmacro defgeneric (function-name lambda-list &rest options)
  (multiple-value-bind (opts methods)
...
(defun canonicalize-defgeneric-options (options)
  (let ((canon-opts (mapappend #'canonicalize-defgeneric-option options))
	(res-opts nil)
	(methods nil))
    (do ((opts canon-opts))
	((null opts))
      (let ((key (pop opts)))
	(if (eq key :method)
	    (push (pop opts) methods)
	    (progn (push key res-opts)
		   (push (pop opts) res-opts)))))
    (values (reverse res-opts) (reverse methods))))
...
;; MG
(defvar *print-object-old* #'cl:print-object)

(defgeneric print-object (instance stream))

;; MG
(defmethod print-object ((obj t) stream)
  (funcall *print-object-old* obj stream))
